...
app.openapi(
  ...
  async (c) => {
    ...
    // ── 單日模式：課表候選 + 既有記錄 ─────────────────────────
    //
    // ⚠️ 這裡**沒有濾停課**（`sessions.status = 'cancelled'`），所以停課班級的學生
    // 仍然會出現在那天的訂餐候選裡。
    //
    // **而那可能是對的** —— 停課那天仍然可能供餐（有些補習班照出）。
    // **但沒有人決定過**：這一行不是刻意保留，是**從來沒有被問過**。
    //
    // 這跟「停課只改一個欄位」那族的其他下游**形狀不同**，所以不要照它們修：
    // 那些是**自動寫入或自動計數**（掃碼寫出勤、扣堂數、請假連動），
    // 這裡是**給人挑的候選清單**，餐點出不出是行政按下去的。
    //
    // **觸發條件：使用者第一次問起「停課那天的訂餐」時。**
    // 那時候答案會由他給，不需要我們猜。（issue #568 掃描時查到，計畫席裁定
    // 記理由不開工單 —— 它不阻塞任何人。）

...
describe('resolveTeacherScope', () => {
  it('管理員不受限，沒指定就是全部', () => {
    expect(
      resolveTeacherScope({ roles: ['admin'], requested: undefined, ownStaffId: null }),
    ).toEqual({ teacherId: undefined });
  });

  it('管理員可以指定看某位老師', () => {
    expect(resolveTeacherScope({ roles: ['admin'], requested: OTHER, ownStaffId: null })).toEqual({
      teacherId: OTHER,
    });
  });
  ...
  it('老師沒有對應的 staff 列時拒絕，而不是放行', () => {
    expect(
      resolveTeacherScope({ roles: ['teacher'], requested: undefined, ownStaffId: null }),
  ...
  it('既不是管理員也不是老師一律拒絕', () => {
    expect(
      resolveTeacherScope({ roles: ['parent'], requested: undefined, ownStaffId: null }),
    ).toEqual({ forbidden: true });
  });
});

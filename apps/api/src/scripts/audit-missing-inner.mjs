 ...
 *
 * ## 刻意不做的反方向：「有 `!inner` 但沒有條件」
 *
 * 那**不是**多餘。`!inner` 的另一個正當用途是**保證關聯存在** —— 例如
 * `sessions.ts` 列表的 `classes!inner`：它身上沒有任何 `.eq('classes.…')`，
 * **但它是對的**，因為課堂一定屬於某個班，用 `!inner` 表達那個不變量。
 *
 * 所以這支的偵測是**由條件驅動**的（先找 `'表.欄位'` 形狀的條件，再看那個 embed
 * 有沒有 `!inner`）—— 沒有條件的 embed 根本不會進入判斷。**要是反過來做成
 * 「掃出所有沒被條件用到的 `!inner`」，那份清單會把正確的不變量宣告全部誤報進去。**
 *
 * （這一點由 billing-api 指出：我們兩席都是從 `sessions`/`events` 這一帶歸納出

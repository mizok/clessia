...
## 6. 原版那三條保護規則的下場（2026-09-02 裁決）

除鏽時對照程式碼，發現原版第 5 節與 2.1 寫的三條規則都跟實作對不上。
計畫席當天分流完畢，結論不一樣，所以分開記：

### 6.1 「系統推算不得覆蓋人工修改」—— 碼錯，待修

`daily-checkins.ts` 的 upsert 是 `ignoreDuplicates: false`，所以後來的掃碼會
**覆蓋**先前的人工修改，把狀態寫回 `present`：老師改成缺席、學生下午補掃碼，
紀錄就變回出席，而且沒有痕跡。

**這條是資料完整性問題，已派 billing-api 席。** upsert 應該尊重人工痕跡；
要怎麼區分「人工改過」可能需要欄位，由該席提案。

### 6.2 「課堂結束後自動標記 `absent`」—— 文件錯，碼是對的

repo 裡沒有任何排程或批次在做這件事，而**這是對的**：現行語意是
「沒有紀錄 + 課已結束 = 漏點名」，不是「沒有紀錄 = 缺席」。

把沒點名自動寫成一整排 `absent`，會讓「老師忘了點」跟「學生真的沒來」
在資料上變成同一件事，之後再也分不開。原版那句話是虛構的規劃，不是遺漏的實作。

### 6.3 「掃碼只算有報名的課堂」—— 待業務確認

events 只用 org / 日期 / 分校篩選，**沒有比對報名**，所以掃碼會替該分校當天
每一堂課都寫一筆 `present`，包含學生沒報名的課。

**這條沒有被當成 bug** —— 日到班模式下「人來了就算到」也可能是刻意的。
已進使用者的業務問題清單，跟「銷假」一起問實際補習班；答案回來再決定要不要動。

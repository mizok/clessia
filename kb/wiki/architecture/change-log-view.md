...
> ⚠️ **這一節是設計當下（2026-08-12）的起點。** 本頁的處方已經實作，`admin/changes`
> 現在是可用的畫面（`features/admin/pages/changes/`）。

`schedule_changes` 從 2026-02 起就一直在寫（散落在 `courses` / `classes` / `sessions`
三支路由的多個 insert），記錄調課、代課、停課、取消停課、改時間。**但當時
`admin/changes` 是空殼，沒有任何畫面看得到這些紀錄。**

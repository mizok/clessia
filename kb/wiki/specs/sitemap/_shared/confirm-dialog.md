...
結構事實：`shared/components/confirm-dialog/confirm-dialog.component.ts` **沒有注入任何 service**
（`grep "Service)"` 回空）。它拿到的是呼叫端傳進來的文案與 callback，
**確認之後的那支請求屬於呼叫端**，不屬於這支對話框。

所以它沒有載入中、也沒有錯誤態。**它的失敗態是呼叫端的失敗態。**

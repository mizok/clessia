...
**開啟時不適用 —— 這支對話框開起來不取任何資料。**

實測（`/admin/contact-book`，把日期區間挪到 `2026-08-25 ~ 2026-09-01` 才有列可點，
見下面「怎麼讓它有資料」）：

| | 結果 |
| --- | --- |
| 攔到的請求 | **0**（延遲模式與失敗模式都是） |
| `.p-dialog` | 1 支，內容完整：`出勤測試學生01／2026-08-29 · 最後由 張品妍 編輯／未簽收／內容／取消／儲存` |
| skeleton / spinner / toast | 0 / 0 / 0 |

⚠️ **請求數 0 本來要作廢那一輪** —— 這裡附了正控：**同一次量測裡對話框確實渲染出真實內容**
（學生名、日期、最後編輯者都對得上 DB），所以 0 是這支對話框的性質：
**它的資料是呼叫端從清單傳進來的，不是自己去撈的。**

**錯誤態仍然未驗**：唯一會失敗的請求在「儲存」後面，而那是寫入類動作（Phase 2-D 一律不按）。

### 怎麼讓它有資料（先前兩個開啟點都開不到）

`contact_book_entries` 全庫只有 **6 筆**，`entry_date` 是 **2026-08-28 / 08-29**，
而 `/admin/contact-book` 預設區間是最近七天 —— **所以預設狀態下一列都沒有。**

把區間挪過去（唯讀）：

```js
// 頁面元件的 dateRange 是普通陣列欄位，不是 signal；要走它的 handler
cmp.onDateRangeChange([new Date(2026, 7, 25), new Date(2026, 8, 1)]);
```

之後清單出現 6 列（`出勤測試學生01/02/03` 各兩天），點任一列即開啟。

---
title: 整站 UI 地圖 —— 方法頁
summary: 怎麼畫一頁 UI 地圖、怎麼用瀏覽器兩向比對驗證它，以及十四個會讓驗證靜靜失效的坑。
category: spec
status: developing
tags: [sitemap, method]
created: 2026-09-12
...
## Phase 2-B：寫入實按 —— 沒有 `db:reset` 的時候怎麼做

工單的前提是「本機 DB 可拋棄、`db:reset` 就回來」。**那句話技術上為真、政策上為假**：
`.claude/settings.json` 的 `deny` 擋掉 `db:reset` 的五個變體（含 `rtk` 包裝），
`.claude/settings.md:72` 寫明理由是「**會清掉本機資料與 seed**」。

> **檢查點：工單的方法裡如果有一個「出事就靠它復原」的動作，先確認那個動作真的做得到。**
> 這跟「工單的前提是觀察還是類比」是同一族 —— 這一次前提是觀察來的（DB 確實可拋棄），
> **但它沒有被拿去對照專案自己的規則。**

### 做法：只按「可自行復原」的動作

**建立一筆自己的測試資料 → 對它走完整生命週期 → 刪掉。不碰任何 seed 資料。**
識別字統一前綴（本輪是 `QA-758-R<輪次>`），這樣：

- 量完可以一句 SQL 確認殘留是 0
- 下一輪真的 reset 時，它是比 `oid` 更貼近你在意的事的哨兵 ——
  **它證明的不是「表被重建」，是「我上一輪弄髒的東西真的沒了」**

**組織級的單值設定（例如 `attendance_mode`）用「來回」**：改成另一個值 → 驗 → 改回去 → 驗。

**這樣做不到的那一半要明講**：所有「單向」的動作（發布、收款、批次生成、
以及**「有子資料所以刪不掉」那種要拿 seed 資料去撞的分支**）**一律不按** ——
不是因為麻煩，是因為**萬一它其實成功了，沒有東西可以救**。

### ⚠️ 不要驗證一個「失敗時要靠被驗證的東西來收拾」的方案

本輪查過一條繞開 deny 的替代路：容器內 `pg_dump`（`docker exec supabase_db_clessia`，
17.6，本機的 14.17 版本不合）。**dump 與「還原到拋棄式 DB」都驗過了**
（`campuses 13/13`、`schools 25/25`、`subjects 9/9`、`students 70/70`、`enrollments 98/98`，
8 個 `pg_restore: error` 全是 Supabase 基礎設施、不碰業務資料）。

**但「還原回正式的那個庫」沒有驗，而且刻意不驗**：

> 要驗證 `pg_restore --clean` 打在 live DB 上安不安全，就得真的打一次；
> **而萬一它中途失敗，收拾的手段正是那條被 deny 的 `db:reset`。**
> **驗證失敗的退路等於被驗證的東西本身 —— 這種驗證不要做。**

（計畫席另外裁定：用 `pg_dump` 繞過安全欄，跟拿掉安全欄是同一件事，只是更難審。）

### 表單有兩種輸入，只有一種吃得下合成的值 —— 而另一種是**靜靜**吃不下

| 種類 | 「原生 setter + 派 `input`」 |
| --- | --- |
| **原生的**：`pInputText`、`input[type="time"]`、`textarea` | ✅ 有效 |
| **PrimeNG 包裝的**：`p-datepicker`、數字輸入 | 🔴 **無效** |

**失敗形狀最壞的那一種**：DOM 上讀得到你填的值（`input.value` 就是你設的），
**送出去的是舊值**，而畫面不會抱怨。我因此兩次把「填了沒存進去」當成缺陷候選
（班級的起訖日期、人數上限），查了才知道是自己。

**兩條退路**：

1. **點它自己的 UI** —— `p-datepicker` 的日曆點得動（`panel.querySelectorAll('td')` 找到那一格，
   點 `td` 裡的 `span`）。實測有效。
2. **從元件 signal 設值** —— `ng.getComponent(el)` 拿實例，`cmp.someSignal.set(值)`。
   ⚠️ **那是「設模型」不是「按 UI」，證據等級要另外標** —— 它跳過了 UI 的輸入驗證。

⚠️ **第二個 overlay 在合成事件下開不起來**（第一個可以）。同一個表單裡兩個
`p-datepicker`，第一個 `.click()` 開得了、第二個怎麼點都不開。
**成因與坑 12 補的那則同族**：`transitionend` 不觸發 ⇒ CDK overlay 收不乾淨 ⇒ 下一個開不了。
**遇到就走退路 2，不要以為是產品的問題。**

### 每一顆按完查一次 DB —— 因為「畫面沒說」不等於「沒發生」

本輪最直接的實例：`subject-manager` 的**改名成功完全沒有提示**
（原始碼確認只有失敗才 `showNotice`）。只看畫面會把它記成「按了沒反應」，
**查 DB 才知道它成功了**。

反方向也要查：toast 說成功、而資料沒落地，同樣只有 DB 分得出來。
...
### 13. `getComputedStyle` 讀到的那個數字是死的，而它看起來合理

2026-09-13（#848 的重掃）：驗「觸控下的展開鍵不再是圓鈕」時，
`border-top-width` 從 `1px` 變 `0px`、`border-radius` 從 `9999px` 變 `0px`、
`box-shadow` 變 `none` —— **三個都跟著改了，只有 `backgroundColor` 卡在
`oklab(0.99791 0.000122398 0.000208974)` 不動。**

排除的東西全部排除過了：沒有 `!important`（掃過所有命中的規則）、
`adoptedStyleSheets` 是 0、CSSOM 裡那條 `background: none` 確實存在且是最後一條。
**最後一次測試才是決定性的**：

```js
b.style.backgroundColor = 'rgb(255,0,0)';
getComputedStyle(b).backgroundColor; // 還是 oklab(0.99791 …)
```

**inline 樣式只會輸給 `!important`。** 它連紅色都吃不下，代表那不是層疊問題 ——
**是那個讀數本身死了**，而它回傳的是一個看起來完全合理的顏色值。

> **這一則跟坑 6、坑 12 同族：環境少了一項能力，而缺席的樣子跟「產品就是這樣」一模一樣。**
> 但它的方向是**反的** —— 坑 6 讓你以為元件壞了，這一個讓你以為**樣式沒生效**，
> 而那正是你當下在驗的東西。「樣式沒套上」與「讀數壞了」在數字上一字不差。

**處置：截圖。** 這一次是畫面上根本沒有白色圓底才分得出來。

> **可操作**：驗「某個樣式有沒有生效」時，**先對那個屬性做一次正控** ——
> 用 inline 設一個絕不可能被誤認的值（紅色、999px），確認讀得到變化。
> 讀不到就換工具，不要繼續讀那個數字。
>
> 這是 README 那條「驗證通過時，問它驗的是不是你以為的那件事」的**量測層版本**：
> 前者問的是語意，這一條問的是**儀器還活著嗎**。

### 14. 掃描器不認得「正當的例外」，於是它跟「真的壞掉」長得一模一樣

#848 的版面掃描有一條判準是「`responsive-table` 收合後，宣告順序第一個非動作欄
有沒有被收掉」。修完重掃，`/admin/contact-book` **還是報 ✗** ——
而那一頁是**對的**：它第一欄是日期，身分是學生，所以它正當地 opt-out 了主欄預設值。

有兩條路：

| 做法 | 代價 |
| --- | --- |
| 掃描器裡加一張白名單 | 白名單會腐化，而且**它記的是「這一頁不要報」，不是「為什麼」** |
| **把不變量暴露到 DOM** | directive 加一行 `@HostBinding('attr.data-rt-col-primary')` |

選後者。掃描器改讀那個屬性（`dc.find((c) => c.p) ?? dc[0]`），
於是**它問的問題從「第一欄有沒有被收掉」變成「這張表宣稱的主欄有沒有被收掉」** ——
後者才是那條規則真正要守的東西。

> **判準：報告要分得出「正當的例外」與「真的壞掉」。分不出來的時候，
> 先問那個不變量能不能被外面看見，再考慮白名單。**
> 白名單讓例外**消失**在報告上；暴露不變量讓例外**顯示成例外**（這裡是 `✓★student`）。
> 差別在下一個人讀報告時看到的是「沒事」還是「這裡有一個刻意的決定」。
>
> 附帶的好處：那個屬性同時讓「主欄永不收合」這條規則**在瀏覽器裡可驗**，
> 而不是只活在 `computeColumnVisibility` 的單元測試裡。

...
**不取列表資料。** 實測：裝好攔截之後靜置 3 秒，**請求數 0**。
與 Phase 1 讀元件得到的結論一致（「這一頁不打任何列表 API，只在按下去時才呼叫 `auth.linkLine()`」）。

> ⚠️ **證據等級**：攔截是在**頁面載入完成之後**才裝的，所以這一輪證明的是
> 「**載入之後不再取數**」，不是「載入時不取數」。後者靠 Phase 1 的元件查證，
> 兩者方向一致但不是同一件事。

**錯誤態**：唯一會失敗的請求是按下「綁定 LINE 帳號」之後的 `auth.linkLine()`，
而那是**寫入類動作，一律不按**（Phase 1 已記）。所以這一頁的錯誤態**仍然未驗**。

> **2026-09-13（labor-8）：「載入中」那一半同樣不適用** —— 頁面載入之後請求數 0，
> 沒有可以延遲的請求。**錯誤態仍然未驗**，理由不變（唯一會失敗的請求在寫入類動作後面，
> 一律不按）—— 那要等 #758 的窗口，而 #758 也不會按它（它會離開本站）。

---
title: 續課資訊（/parent/renewal）
summary: /parent/renewal 的實際 UI 地圖：目前是 EmptyState 佔位頁，<main> 內零互動元素。
category: spec
status: developing
tags: [sitemap, parent]
created: 2026-09-12
...
1. 圖示 `pi pi-refresh` —— 來自 `page().icon`，跟選單用的是同一個（#708）
2. 標題 **續課資訊** —— 來自 `page().label`，也就是 `RoutesCatalog` 的路由標籤
...
`<main>` 的全部文字：`續課資訊 這個功能還在準備中，完成後就會出現在這裡。`
空狀態圖示的實際 class：`pi pi-refresh`（**#708 修正前**；現在是 `pi pi-refresh`）

---
title: 試聽申請（/parent/trial）
summary: /parent/trial 的實際 UI 地圖：目前是 EmptyState 佔位頁，<main> 內零互動元素。
category: spec
status: developing
tags: [sitemap, parent]
created: 2026-09-12
...
1. 圖示 `pi pi-star`
2. 標題 **試聽申請** —— 來自 `page().label`，也就是 `RoutesCatalog` 的路由標籤
...
`<main>` 的全部文字：`試聽申請 這個功能還在準備中，完成後就會出現在這裡。`
空狀態圖示的實際 class：`pi pi-star`
...
圖示 `pi pi-star` 寫死在元件的 `template` 字串裡，**不是** `page().icon`。
這條路由在 `RoutesCatalog` 的圖示是 `pi-headphones` —— **兩者不同**，

---
title: 餐費紀錄（/parent/meals）
summary: /parent/meals 的實際 UI 地圖：目前是 EmptyState 佔位頁，<main> 內零互動元素。
category: spec
status: developing
tags: [sitemap, parent]
created: 2026-09-12
...
1. 圖示 `pi pi-shopping-bag`
2. 標題 **餐費紀錄** —— 來自 `page().label`，也就是 `RoutesCatalog` 的路由標籤
...
`<main>` 的全部文字：`餐費紀錄 這個功能還在準備中，完成後就會出現在這裡。`
空狀態圖示的實際 class：`pi pi-shopping-bag`
...
圖示 `pi pi-shopping-bag` 寫死在元件的 `template` 字串裡，**不是** `page().icon`。
這條路由在 `RoutesCatalog` 的圖示是 `pi-dollar` —— **兩者不同**，

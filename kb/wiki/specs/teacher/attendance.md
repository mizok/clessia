...
- 跨日的假在按下去**之前**就先講「此假到 X（不只今天）—— 銷假可能一併取消後續日期」。
  **今天才開始的假不警告**（#222）：`leaveStartDate` 是聯集的最小值，它等於這堂課的日期
  就代表沒有任何一張假更早開始，每張都只會縮到明天或被刪掉，零損失
- **文案是「可能」不是「將」，這是刻意的。** 兩張假接力蓋到今天時
  （`[4/4~4/6]` + `[4/6~4/8]`，今天 4/6），聚合值 `start=4/4, end=4/8` 跟一張真長假
  長得一模一樣，但實際損失是零。要說死需要後端回真正的預測值（等同 dry-run 的
  `droppedAfter`）。在那之前寧可保守 —— 說死了卻沒發生，下次就沒人信這句話

...
統一登入端點 `POST /api/login` 已實作，支援 email 和手機號碼雙模式。
登入頁 UI 加入 Email / 手機號碼 Tab 切換。
...
_Phase 2 已全部完成。課堂管理含批次操作（指派/調時間/停課/恢復）、調課歷史、代課、audit log。_
...
| 家長管理 | `kb/wiki/specs/admin/academic/parents.md`      |
| 出勤管理 | `kb/wiki/specs/admin/attendance/attendance.md` |
...
規格：kb/wiki/specs/admin/system/staff.md
路徑：/admin/staff
依賴：分校（已完成）

...
- [[architecture/line-oauth-login]] — 密碼雜湊超過 Cloudflare Workers 免費方案的 10ms CPU 上限，登入間歇性 503。改用 OAuth 取代日常密碼登入（首發 LINE，Google 延後但架構預留），root 保留密碼作為破窗管道；OAuth 身分靠一次性綁定連結／QR 對應到既有的人員或家長記錄。

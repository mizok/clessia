...
-- =============================================================================
-- ===== 「展示狀態補齊」（#685）已搬到 `seed-demo.sql`（#838）=====
-- =============================================================================
--
-- 它指名依賴的四樣（`張宇軒` / `范芷寧` / `文山旗艦校` / `國三數學 A 班`）
-- **是 `seed-demo.sql` 造的**，而 `db:reset` 只跑本檔 —— 所以它在這裡必定 RAISE
-- EXCEPTION 並讓整批 rollback（#838，本機因此變成空庫一次）。
--
-- `config.toml` 的 `sql_paths` 現在是 `["./seed.sql", "./seed-demo.sql"]`，
-- 兩支都會被 `db:reset` 套上，順序是本檔先（`seed-demo.sql` 依賴本檔的帳號）。
--
-- **下一個要加「依賴展示資料」的段落請加在 `seed-demo.sql`，不要加在這裡。**
-- 本檔是冒煙測試用的（每張表 1–6 筆），而 `test-baseline.json` 的基線依賴它。
-- =============================================================================

...
| 頁面                     | 路徑                | 功能                                         |
| ------------------------ | ------------------- | -------------------------------------------- |
| 課程日曆                 | `/admin/calendar`   | 日曆顯示所有課堂，可進入課堂詳情（詳見下方） |
| **出勤管理**（收合選單） |                     |                                              |
| ├ 出勤歷史紀錄           | `/admin/attendance` | 查看/修正學生出勤狀態                        |
| └ 請假管理               | `/admin/leave`      | 建立/查詢請假紀錄                            |
| 餐費管理                 | `/admin/meals`      | 新增/查詢餐費紀錄、匯入匯出                  |
| 成績查閱                 | `/admin/grades`     | 查詢學生成績                                 |
| 課堂搜尋                 | `/admin/sessions`   | 依條件搜尋課堂                               |
| 課務異動紀錄             | `/admin/changes`    | 查看調課/代課/停課紀錄                       |
...
| 頁面         | 路徑                   | 功能                                       |
| ------------ | ---------------------- | ------------------------------------------ |
| 分校管理     | `/admin/campuses`      | 建立/編輯分校、設定出勤模式                |
| 課程管理     | `/admin/courses`       | 建立/編輯課程                              |
| 開課班管理   | `/admin/classes`       | 建立開課班、設定上課時間、產生課堂         |
| 費用方案管理 | `/admin/fee-templates` | 設定開課班的收費標準（詳見下方）           |
| 學生報名     | `/admin/enrollment`    | **直接把學生加入開課班**（跳過申請流程）   |
| 人員管理     | `/admin/staff`         | 建立老師/管理員帳號                        |
| 系統設定     | `/admin/settings`      | 試聽跟進、繳費、補習班資訊設定（詳見下方） |

...
- 描述系統的世界觀、角色、功能、業務規則。
- **不包含**技術實作細節（那些在 TECH.md）。
- 所有後續的技術文件都必須符合這份 PRD。
- 如果 PRD 和 TECH 有衝突，以 PRD 為準。
...
2. **例外處理麻煩**：忘記打卡、臨時請假、事後補請，行政處理起來很累。
...
- **待審核（pending）**：已提交，等待管理者審核。
- **待付款（awaiting_payment）**：審核通過，已建立繳費單，等待繳費。
- **已完成（completed）**：繳費完成，帳號和資料已建立，報名正式生效。
- **已拒絕（rejected）**：管理者拒絕申請（需填寫原因）。
- **已取消（canceled）**：家長取消未審核申請或行政代為取消（僅限 `pending`）。
...
- 系統依開課班設定的學費自動計算應繳金額。
- 學期中加入可按比例計算（依剩餘課堂數）。
- 管理者建立繳費單時可調整金額（如早鳥優惠、兄弟姊妹折扣）。

**繳費期限**：

- 建立繳費單時設定繳費期限（預設 7 天）。
- 超過期限未繳費，申請狀態自動變為「已過期」。
- 已過期的申請可由管理者重新啟用（延長期限）。
...
- 系統自動執行以下動作：
  ...
  4. **建立家長帳號**（若為新生）
     ...
     - **若無 Email**：系統產生「帳號資訊卡」，由行政人員列印或拍照提供給家長。
...
3. **部分付款時（部分沖銷與強制啟用）**：
   - **彈性啟用**：若收款金額小於應繳總額，管理者在確認收款時可指定「本次欲啟用的課程項目」。
   - **強制啟用 (Force Activation)**：即使該課程項目的金額尚未完全付清（例如分期付款或賒帳），管理者仍可強制啟用該課程的 Enrollment。
   - **狀態處理**：
...
4. **失效恢復 (Reactivation)**：
   - 若繳費單因逾期變為 `void`，但家長隨後補繳（線下付款），管理者確認收款後。
...
管理者可自訂任意數量的繳費週期，系統不預設固定的「上學期/下學期」結構。
...
> 模板僅為快速起步，管理者可隨時新增、修改、刪除任何週期。月繳學生不受週期設定影響，固定依系統設定的月繳發送日期處理。
...
1.  **預告通知（T-5）**：
    ...
    -   若家長在此期間提交異動申請（如轉班、停課），系統將**暫停該生的自動開單**，並產生「處理續課異動」任務供行政人員處理。
...
2.  **自動開單（T-0）**：
    ...
    -   **前置檢查**：
        ...
        2.  **名額檢查 (Capacity Check)**：檢查 `next_class_id` 的剩餘名額。若已額滿 (Enrollments >= MaxStudents)，不開單並產生任務通知行政人工處理。
    ...
    -   針對**已申請異動**的學生，跳過自動開單，待行政確認並處理任務（如轉班）後，**需手動開立**正確的繳費單。

3.  **繳費與失效**：
    -   家長收到繳費單通知後進行繳費。
    -   **自動失效 (Auto-Void)**：若超過繳費期限與寬限期（Grace Period）仍未繳費，系統將自動作廢繳費單並暫停學生權限。行政人員無需逐一確認是否不讀，僅需處理之後想復學的個案。

**主要優勢**：
-   **低摩擦**：95% 的家長無需任何操作，自動收到帳單並繳費。
-   **高彈性**：保留緩衝期讓家長提出異動，避免開出錯誤帳單。
-   **自動化**：透過自動失效機制解決「幽靈帳單」問題，大幅降低行政負擔。
...
- **待處理（pending）**：已提交，等待行政安排試聽。
- **已安排（scheduled）**：已安排試聽時間，等待學生參加。
- **已完成（completed）**：試聽結束，已電話跟進確認意願。
- **已取消（canceled）**：家長致電補習班，由行政/管理者代為取消。
...
- **角色可自由組合**：同一個人可以同時擁有任何角色組合（例如：管理者 + 老師 + 家長）。
- **家長兼任處理**：
  - 若使用者同時擁有「家長」與「非家長（管理者/行政/老師）」角色。
  - 登入時預設進入「工作視圖」（管理者/行政/老師介面）。
...
- 電話詢問家長意願
- 在系統記錄結果：
  - **要報名**：告知家長填寫報名表單（`/apply`）或由行政直接建立。當系統檢測到報名建立後，將自動標記試聽完成。
  - **不報名**：記錄原因，標記試聽完成
  - **考慮中**：記錄備註，可稍後再跟進

**說明**：
- 試聽申請與報名申請流程獨立。若家長試聽後決定報名，需填寫報名申請表單（系統可自動帶入試聽資料）。
- **自動關閉邏輯**：當系統建立新的報名申請（Enrollment Request）且該學生/家長資料與「進行中的試聽申請」匹配時，系統應自動將該試聽申請標記為「已完成」（若尚未完成），並自動取消或完成關聯的「試聽跟進」任務，避免行政重複作業。
...
- 管理者直接報名可選擇是否產生繳費單，方便處理已現場繳費或特殊免費情況。
...
- 狀態時間軸：顯示申請的狀態變化歷程
- 若需取消，請電話聯繫補習班，由行政/管理者代為取消（不提供家長自助取消）
...
- **狀態提示**：
  ...
  - 異動處理中：「您已提交異動申請，行政人員處理中，自動開單已暫停。」
...
- **三欄輸入區**（皆使用 Markdown 編輯器）：
  - **課程範圍**：記錄本堂課教學進度（例如「第三章 3-1~3-2」）。
  - **作業**：記錄指派的作業內容與繳交期限。
  - **備註**：記錄其他補充說明（學生表現、提醒事項等）。
- **編輯器特性**：
  - 採用「所見即所得」風格的 Markdown 編輯器。
  - 提供工具列按鈕：粗體、斜體、列表、連結、標題。
  - 支援兩種輸入方式：點擊工具列按鈕，或直接輸入 Markdown 語法（如 `**粗體**`、`- 列表`）。
  - 輸入 Markdown 語法會即時渲染為格式化文字。
  - 背後儲存為 Markdown 格式，方便未來擴充。
  - 桌面版可選擇分割視圖（編輯 + 預覽）。
  - 手機版使用單一編輯視圖 + 工具列。
- **附件區**：
  - 顯示已上傳的附件列表，每筆顯示檔名、檔案類型圖示、移除按鈕。
  - 「上傳檔案」按鈕新增附件。
  - **附件限制**：
    - 每筆聯絡簿最多 3 個檔案。
    - 單檔大小上限 5MB。
    - 允許格式：圖片（JPG、PNG、GIF）、影片（MP4）、文字檔（TXT、PDF）。
  - 上傳後家長可在課堂詳情 Popup 中下載附件。
- **操作按鈕**：
  - 「取消」：關閉 Popup，不儲存變更。
  - 「儲存」：儲存聯絡簿內容並關閉 Popup。
- **桌面版**：寬版 Popup（max-width: 600px）。
- **手機版**：全螢幕 Modal，三個輸入區域垂直排列。
...
| 待處理工作               | `/admin/tasks`      | 查看、認領、處理任務（與管理者共用頁面設計） |
...
- **管理者**：可使用所有功能，包括系統設定、人員管理。
- **行政**：可使用行政頁面功能，不能管理人員和系統設定。
- **老師**：只能使用老師頁面功能（課表、聯絡簿、成績）。

**學生報名頁面**：

這是將學生加入開課班的關鍵功能，沒有報名就不會出現在課堂名單中。僅管理者可操作，避免繞過金流。
...
- 行政/管理者可在「家長管理」頁面重新產生帳號資訊卡（用於密碼遺失或帳號變更）。
- 重新產生會重設密碼並產生新卡片。
...
│ ▼ 操作紀錄                                  │
│ ─────────────────────────────────────────── │
│ 2/5 14:30  李行政  確認收款（現金）         │
│ 2/3 10:15  李行政  建立繳費單 $18,000       │
│ 2/2 09:30  張管理  審核通過                 │
│ 2/1 08:00  系統    收到報名申請             │
└─────────────────────────────────────────────┘
```
...
│ ▼ 出勤修改紀錄                              │
│ ─────────────────────────────────────────── │
│ 2/5 10:30  李行政  李小華：缺席 → 請假      │
│ 2/5 10:25  李行政  陳大文：出席 → 遲到      │
└─────────────────────────────────────────────┘
```
...
- **課堂模式**：學生打卡只記錄到班時間，出勤需由行政或任課老師逐堂確認。
...
- 管理者、行政：可修改所有課堂的出勤，不限時間。
- 老師：只能修改自己任課課堂的出勤，**僅限當天**。過期的出勤修改需找行政處理。
...
| 事件         | 通知對象             | 通知內容                                        |
| ------------ | -------------------- | ----------------------------------------------- |
| 新任務產生   | 同分校的管理者和行政 | 「有新的[任務類型]待處理：[關聯對象]」          |
...
| 續課確認完成 | 管理者、行政 | 「[家長姓名] 已確認 [學生姓名] 的續課」                |
| 續課確認逾期 | 管理者、行政 | 「[學生姓名] 的續課確認已逾期，請跟進」                |

**通知一般規則**：

1. 通知保留 90 天。
2. MVP 站內通知 + 推播（僅關鍵事件：續課預告/繳費通知/課務異動）。
3. 首次開啟家長/老師/行政端時請求推播授權，可在設定中關閉。
...
2. 行政認領任務，電話聯繫家長，安排試聽時間，狀態改為「已安排」
3. 學生到班試聽，試聽完成後狀態改為「已完成」
4. 系統產生「試聽跟進」任務，行政電話跟進了解家長意願
...
| 事件       | 系統動作             | 通知對象     | 說明                       |
| ---------- | -------------------- | ------------ | -------------------------- |
| 新試聽申請 | 產生「試聽安排」任務 | 行政         | 行政認領後聯繫家長安排試聽 |
| 安排試聽   | 更新申請狀態         | 家長（電話） | 人工通知試聽時間、地點     |
| 試聽完成   | 產生「試聽跟進」任務 | 行政         | 行政認領後電話跟進確認意願 |

**試聽當日流程**：

- 試聽生到班時由櫃檯行政接待。
- **不需要**進行掃碼簽到。
- 試聽生**不會**出現在老師的常規 APP 點名表中（或由行政口頭告知老師）。
...
- **已繳清 (Paid)**：全額繳清。
- **逾期 (Overdue)**：超過繳費期限，但仍在寬限期內。家長/行政會看到催繳警示，但學生**權限不受影響**（可正常出勤）。
- **已失效 (Void)**：超過繳費期限 + 寬限期。報名失效，學生無法入班/點名，需行政人工介入處理（重啟或釋出名額）。
...
| 繳費期限即將到期（3天前） | 管理者、行政 | 站內       | 「XXX 繳費單即將到期，請跟進」       |
...
| 自動失效通知 | 家長 | 您的報名已因逾期未繳而自動取消。如需復學請聯繫行政。 |

**人工介入時機**：

1.  收到家長提交的「異動申請」任務時。
2.  家長錯過所有通知，導致被「自動失效」，打電話來要求復學時（行政手動重啟繳費單）。
...
管理者/行政可將帳號密碼重設為**系統隨機生成的 6 位數代碼**：
...
1. 使用者致電補習班
2. 行政/管理者確認身份後，在系統中重設密碼
3. 系統產生一組隨機 6 位數代碼（僅顯示一次），行政人員需透過**口頭告知或線下紙本**等安全方式傳遞給使用者
4. 使用者下次登入時強制修改密碼

...
2. **零 idle 執行**:確認 idle 席收到 backlog 認領提醒;佇列空了通知計畫席補貨。
   ...
   **這條每輪對每一席都要做,不是只對 idle/done 的席做**(2026-09-06 計畫席二次
   提出同一件事):全隊曾經 idle 五個半小時,期間每一輪巡檢都回報「七席正常」——
   那個回報是真的,因為當下 `herdr agent list` 看到的是 working/done 交替,沒有
   連續 5 分鐘的 idle 觸發職責 1。**herdr 狀態量的是「這一刻在不在動」,不是
   「佇列還有沒有貨」**,兩者在一個席位還在做佇列最後一項時看起來一模一樣。
   所以佇列深度掃描不能掛在職責 1 的「idle 超過 5 分鐘」條件下面才做,要當成
   跟帳面巡檢同一層級、每輪都跑的獨立檢查。
   ...
   **「等使用者」若是唯一理由,要反問一次「真的需要使用者,還是計畫席漏派了?」**
   ...
   backlog 裡只寫「等使用者」卻讓佇列整個空掉,大機率是漏派而不是真卡在使用者。

   數的時候兩件事容易算錯,首輪就各錯了一次:
   - **大刀權重算 2**:feature-slice 主刀、查證報告類這種本身能吃好幾天的項目,
     單獨一項就不該觸發警報 —— 深度是「還能撐多久」的代理指標,不是行數。
   - **活項要跟實際交付對帳,不能只看文字有沒有 `~~`**:2026-09-05 把 infra 的
     「gate 載體盲區掃描」算成活項,但那筆早就靠 #296 交付掉了,backlog 那行只是
     沒同步。這行文字本身**沒有引用任何 PR 編號**,所以職責 4 的「掃引用的 PR
     編號」根本碰不到它 —— 抓不到的不代表沒問題。數深度前,對看起來眼熟的項目
     順手 `git log --oneline --grep <關鍵字>` 或問一句「這行說的事,是不是已經在
     某支 PR 裡做掉了」,不要純算行數。

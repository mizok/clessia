...
- **堂數制的產品決定**(session-packs 查證結論):後端+業務規則完整,前端零頁面 ——
  使用者現在能把報名設成堂數制,但沒有任何畫面能買包/看剩餘/追補買。二選一:
  (a) 排堂數包前端面(規則 1/8 已寫好,做起來有依據);(b) 暫時藏起 billing_mode 的
  session_pack 選項,避免選了無法操作(計畫席建議:若 (a) 不在近期,先做 (b) 止血)

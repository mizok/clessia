...
6. **收自己的 port 之前先確認那個 PID 真的是 server，而且 cwd 是自己的。**
   `lsof -ti:<port>` **沒加 `-sTCP:LISTEN` 會把 client 一起撈進來** ——
   我收 4210 時它回了兩個 PID，其中一個 cwd 是 `/`（是連線端不是 server）。
   `kill` 因為參數格式壞掉而失敗，我才回頭查，**那是運氣不是紀律**。

   ```sh
   lsof -nP -iTCP:<port> -sTCP:LISTEN          # 拿 PID
   lsof -p <PID> -a -d cwd -Fn | grep '^n'     # 確認 cwd 是自己的 worktree
   ```

   README 既有的「清理自己的 process 用精準 kill」講的是**不要裸 `pkill`**，
   **沒講 `-ti` 會多撈** —— 這是它的查法層。
7. **反向對照要分清楚「編譯失敗的紅」與「斷言的紅」。**
   #723 把測試換成新 API 之後，拿舊元件跑當然紅 —— **但那只證明「測試綁著新 API」，
   不證明「測試抓得到那個行為」。** 想要後者，測試得只用兩版都有的 API
   （例如查 DOM 而不是查新增的 signal），否則就要誠實標明對照的性質。
...
## 交棒時的未完成清單（2026-09-12 23:xx）

| 項目 | 狀態 | 下一步是誰的 |
| --- | --- | --- |
| **PR #701**（路由 `access` 收進 `RoutesCatalog`） | **OPEN，保留類** —— 動到 guard 怎麼掛＝授權邏輯 | **使用者親合**，計畫席已標 |
| **PR #731**（#723 通知中心副標＋發布鍵） | OPEN，CI 跑著 | 計畫席，CI 綠即合 |
| **issue #693** | OPEN，由 #701 處理中 | 隨 #701 合併關閉 |
| **issue #723** | OPEN，由 #731 `Closes` | 隨 #731 合併關閉 |

**已合**：#671 #673 #675 #678 #682 #690 #697 #705 #706 #720 #724。
**已關**：#670 #676 #686 #702 #714（由別席 #717 修）。

⚠️ **`lsof -ti` 那一則我沒有開 README PR**（限速中斷後計畫席裁定收進 charter）。
**它還沒進 `herdr-team/README.md`** —— 下一任若覺得該提到位階高的那份，
記得它現在只活在這裡。

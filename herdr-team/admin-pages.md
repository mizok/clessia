...
## 進行中狀態（2026-09-05 輪替交接 —— 這節會過期，接手第一件事：重寫它）
...
| 等什麼                     | 誰在做                                                           | 我這半的狀態                                                                                                                               |
| -------------------------- | ---------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ |
| **及格線欄位**             | billing-api（migration + API）                                   | **前端已就緒（#319）** —— `isFailingScore` 吃 `{ passScore, totalScore }`，`passScore` 是 optional。**欄位落地後只剩把它從考試回應傳進去** |
| **科目平均改「幾分之幾」** | billing-api（`sum` / `totalSum`）                                | **沒有可先行的** —— 顯示「130/150」需要那兩個數字。棄百分比化是使用者裁的                                                                  |
| **家長端 02/03 片**        | billing-api（`childDb` + `activeRole` + `GET /api/me/children`） | 等 #295 的授權模型落地。**02 片是授權的第一個實例，後面每一支會照它抄**                                                                    |
...
- **家長身分在整個 API 層只打得到 `/api/me` 與 `/api/announcements`**，兩支都不回孩子的資料
- 前端 11 頁全是 `empty-state`，但**路由與選單完整**（21 筆 `PARENT_*`）—— 骨架在，內臟沒有
- `parent_student_relations` 從 2026-03 就在了，**關聯有、查詢面沒有**
...
- 已知限制（照現行為出貨，寫在元件檔頭）：`audienceFor` 用角色陣列的優先序決定身分，
  ...
  錯的方向是**靜默漏看**。正解要 `activeRole` 進 API context，在 #295 裡

...
22. **`header:` 配 `showHeader: false` 的對話框設定，那個字串永遠不會被渲染。**
    本 repo 有 31 處這種死設定、18 處是活的，**而兩者在呼叫端長得一模一樣**。
    標題實際來自元件模板裡硬編的 `dialog-header-inline`。#448 那支工單就是這樣寫出來的
    ——回報者看到「出勤紀錄操作紀錄」這種疊字，而**使用者從來沒看過它**。
    確認死活的三個判準：`showHeader` 是不是 false、PrimeNG 的 `getAriaLabelledBy()`
    在 `showHeader === false` 時直接回 `null`（所以也不是無障礙名稱）、有沒有元件
    自己讀 `config.header`。**改對話框文案之前先確認那個字串真的會被渲染。**
    同族還有更陰的一層：**三支 spec 原本在斷言那些死字串**，綠燈證明的是
    「設定值是這個」不是「使用者看到這個」。
23. **`SystemClockService` 的 constructor 會起兩個 `setInterval`。**
    任何用 `vi.useFakeTimers()` + `runAllTimers()` 的元件測試，只要那個元件注入了它，
    就會以「Aborting after running 10000 timers」失敗（`leave.page.spec` 撞過）。
    解法是給替身，範例與理由寫在該 spec 的 provider 旁邊。
    **這條的一般形式**：把一個會自己跑計時器的 root service 注進元件，成本會出現在
    **它的所有測試**上，而錯誤訊息不會提到那個 service。
...
- **「我開檔確認過」只涵蓋你**看到**的那一半，不涵蓋你從它**推出來**的那一半 ——
  而兩半常常寫在同一句話裡。**（2026-09-07，billing-api 指出，計畫席升成全席通則）**
  我在 issue #605 寫「四筆都是我開檔確認的，不是推論」。**事實部分是真的**：
  四個 schema 我確實開檔看過，缺什麼值也都對。**而「回應驗證會失敗 → 500」是推論** ——
  中間那一步是「**schema 存在 ⇒ 它會在執行期生效**」，而 `@hono/zod-openapi`
  **只驗 request 不驗 response**，所以它根本不會觸發。
  **那一步沒有被當成需要查證的東西，因為它看起來像常識。**
  更糟的是第二層：**我拿那個沒驗過的推論去替別人排優先序**（「那不是顯示退化，
  是回應驗證整支失敗」），計畫席據此把它排成第一項。
  **反例一直躺在 repo 裡**：`SessionListItemSchema` 從沒宣告 `attendanceTakenAt`，
  而 `mapSession` 一直在回、前端一直在用 —— 回應若真的會被 parse，那些欄位早就被吃掉了。
  **任何時候查都能推翻我，我沒查，因為我不知道那裡有東西要查。**
  可操作的動作：**回報時把「我看到的」與「我因此認為的」分行寫**，後者標成待驗；
  **尤其在拿它去排優先序或催別人之前。**
...
- **回報「我被擋住了」之前，把那個結論本身也當成一個待驗的推論。**
  ...
  結論是錯的** —— 一個瞬間換成一個日曆日只差一個時區，`Intl.DateTimeFormat`
  原生就做得到，零後端相依。
  **「所以只能等別人」是整條推論裡最少被驗證的那一步**，因為它讀起來像結果
  而不像主張。停工的成本比多查五分鐘高得多，而且停工會把錯誤結論外包給計畫席。
  自我檢查：**我說的「做不到」，是「這件事做不到」還是「我想到的那個做法做不到」？**
- **交一份「零命中」的報告之前，先拿一個已知的真案例餵你的掃描器。**
  **零命中跟掃描器壞掉，在輸出上一模一樣。** #530 第一版漏掉了唯一一筆已知真陽性
  （`childDb.from('students', 'id')` 有兩個參數，我的 regex 要求引號後直接收 `)`）。
  兩個方向都要探：**已知真案例必須紅、已知無害案例必須安靜**（誤報比漏報糟）。
  另外：**先想清楚怎麼問對問題，再開始掃**。直接 grep 欄位名得到 80+ 筆幾乎全誤報，
  因為 `is_active` 從 `parents`/`staff` 砍掉但 `campuses`/`schools` 還有 ——
  **必須先解析出「這個欄位屬於哪張表」**，誤報才會從 80+ 降到 0。
...
### 這輪做了什麼（2026-09-06 下半，接在 #447 交接之後）

七支 PR。**其中兩支的工單前提是錯的，查證推翻之後才動手** —— 這一輪最該繼承的
不是程式碼，是那個順序。

| PR   | 做了什麼                     | 值得記的                                                                                                                                                      |
| ---- | ---------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| #462 | 未點名卡明著送 `statuses`    | 工單說「一行」，**實際要兩半**：只讓儀表板明著送，定義只是從 API 端搬到 web 端，落地頁仍吃自己的 `DEFAULT_STATUSES`。要同時讓落地頁**有帶才套用**才算收成一份 |
| #471 | 刪 31 處不會渲染的 `header:` | 見坑 #22                                                                                                                                                      |
| #477 | 考試 N/M 顯示端              | 低級別告警用**中性色不是淡黃色**（色相表示好壞、深淺表示還在等不再等）                                                                                        |
| #496 | 逾期的「今天」改台北         | 我先回報「被 API 擋住」是**錯的**，見下面工作習慣新增第一條                                                                                                   |
| #532 | 管理端另外五處「今天」       | **五處分兩族**：四處是「畫面說錯話」，`contact-book` 的 `missingDate` 是「資料寫錯格」（會寫進 `entry_date`）                                                 |
| #535 | 儀表板現況區補可點訊號       | teacher-pages 用 live 頁面實測抓到的，靜態掃描看不到                                                                                                          |
| #539 | 三筆觸發條件落檔             | 計畫席指示：理由不要留在 PR 描述裡，PR 會被關掉                                                                                                               |

另外交了 **#530 的唯讀稽核**（掃「還有沒有端點 select 已被 DROP 的欄位」）——
report-only 無 PR，結論是 `apps/api/src` 只有 1 筆命中且是已知那筆。
**方法與盲區寫在 issue 留言裡**，下次同類稽核直接抄，不要重想。

### 這一輪最貴的三個教訓（都是自己犯的）

1. **「查得很仔細」跟「結論正確」是兩件事。** #467 我查到 `SystemClockService`
   沒有日期存取器、`/api/system-time` 只回 `{epochMs, iso}`、`todayLocal()` 也是
   本地時間、8+8 個呼叫端 —— **每一項事實都對，結論「這支被 API 擋住」是錯的**。
   一個「瞬間」換成一個「日曆日」只差**一個時區**，不差更多伺服器資料，`Intl` 原生就做得到。
   **坑 #14 的新一型：查到的事實可以全對，而它們支撐不了你下的那個結論。**
   自我檢查：**我下的結論裡，有沒有一步是「所以只能等別人」？那一步通常沒被驗過。**
2. **零命中跟掃描器壞掉，在輸出上一模一樣。** #530 第一版掃描器回「零命中」，
   而它其實漏掉了**唯一一筆已知真陽性**（`childDb.from('students', 'id')` 有兩個參數，
   regex 沒吃到）。救回來的唯一方法是**拿已知的真案例去餵它**。
   沒有那一步，我會交出一份看起來很完整的錯報告。
3. **量之前不要報數字。** SCSS 預算我一開始算成 +600 bytes，重量之後是 **+200** ——
   我拿了同一輪稍早另一支分支的數字當基準，而中間別席合了東西進去。
   **「基準」也會過期**，尤其在合併速率高的日子。

### 上一版「三件事在等別人」——**三件全部到齊了，而兩件沒有人回頭接**

**2026-09-06 逐項查證，不是抄上一版。** 這一節之所以改寫成這樣，是因為
上一版是快照，而快照會腐化成假情報（README：**會過期的狀態欄寫成「怎麼查」
而不是「是什麼」**）。

| 上一版說在等                  | 現況                      | 怎麼查                                                  |
| ----------------------------- | ------------------------- | ------------------------------------------------------- |
| **及格線欄位**                | ✅ 到了，**行政端已接**   | `git grep passScore -- apps/web/src/app/features/admin` |
| **家長端成績 `passScore`**    | ⚠️ **欄位到了，前端沒接** | 見下                                                    |
| **`monthlyAbsentCount` 拆分** | ⚠️ **拆好了，前端沒接**   | 見下                                                    |

**⚠️ 兩筆「條件成立了但沒有人回頭檢查」——這正是 README 那條規則要防的東西：**

1. **家長端成績的及格線**：`apps/api/src/routes/parent/grades.ts:42` **已經回
   `passScore`**（schema 裡有，`ACADEMY_SCORE_SELECT` 也撈了）。但
   `features/parent/pages/grades/grades.component.ts:107` 的註解還寫著
   「`GET /api/me/grades` 目前不回這個欄位」並退化成比例判斷。
   **那是矛盾不是缺口**（README 的判準：兩個消費端、退化後結論相反、使用者
   沒有線索）——及格線設 70 的考試考 65，**行政端顯示不及格、家長端顯示及格**。
2. **`monthlyAbsentCount`**：`routes/parent/attendance.ts:170-171` 現在同時回
   `monthlyAbsentCount`（純缺席）與 `monthlyOnLeaveCount`。
   **上一版 charter 說「目前顯示的是缺席+請假合計」——那句現在是錯的**，
   照它回報會講錯。web 端 `attendance.page.ts:166` 只讀了前者，沒用後者。

**這兩處都在 `features/parent/`，2026-09-06 起是 teacher-pages 的**（#434）。
**不要自己動**——已在 peer 訊息裡告知該席。這裡記著是因為**契約的另一端是我們
共用的 `isFailingScore`**，而下一個查「為什麼兩端判不一樣」的人會先來翻這份 charter。

### 家長端的現況（**已改派 teacher-pages，不要碰 `features/parent/`**）

> **2026-09-06 席位改編（#434）**：家長端三頁改由 teacher-pages 兼任。
> 這一節留著**不是**因為還歸我們，是因為上面那兩筆矛盾的另一端在我們的
> `shared/` 共用函式裡，下一個查「為什麼兩端判不一樣」的人會先翻這裡。
> **舊 charter 若還寫「家長讀取頁歸我」，那條已作廢** —— 依據是 #344
> child-switcher 合進 main 之後「殼是我的」這個理由就不成立了。
...
**不要抄這一段的狀態，直接查**（上一版的快照腐化成假情報，見上面那張表）：

```
gh issue list --label seat:admin-pages --state open
gh pr list --state open --search 'author:@me'   # 帳號共用，用分支名或標題過濾
```

交接當下：#535（儀表板可點訊號）與 #539（觸發條件落檔）等驗收，
**#530 是 report-only 沒有 PR，所以它不會自己關 —— 要計畫席手動關**。

**等 #607 合併後，把 `makeup` 從 `changes.component.ts` 的 `UNFILTERABLE_CHANGE_TYPES`
拿掉**（一行 + 改 `changes.component.spec.ts` 那條斷言）。billing-api 的 #607 已讓
`ChangeLogQuerySchema` 收 `makeup`（實測：修前 400、修後通過）。
**`creation` 要留著** —— 它是 `buildSessionCreationHistory` 合成的，`schedule_changes`
裡沒有那種列，送過去仍然 400，**而那是刻意的不是漏補**。
解除條件也寫在那個常數正上方（規則放在它守護的東西旁邊）。
...
- **web 測試套件只在部分機器時區綠**：`UTC` / `Asia/Taipei` 綠，
  ...
  **web 從來沒被掃過**。CI 跑 UTC 所以它永遠是綠的 ——
  **綠的原因是觀測窗口不涵蓋出錯條件，不是那裡沒問題**。已回報計畫席與 infra
- **`session-advanced-filters-dialog.component.ts:18` 有第三份 `DEFAULT_STATUSES`**
  （#456 的工單只提了兩份）。它守的是「對話框的重設 vs 頁面的重設」，
  跟卡片/落地頁數字不是同一條路徑；修法要嘛把常數提進 `shared/`、要嘛由呼叫端傳入，
  **前者是結構決定**，所以沒有順手做

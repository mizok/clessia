...
### 接下來（Tester 寫入授權下來後派的，UI 這半）

- **餐費「確認名單」100% 失敗，toast 顯示 `[object Object]`**——API 半派給
  billing-api，我這半是失敗之後畫面沒告訴使用者「名單沒有確認」（toast 幾秒
  自動消失、蓋住兩個欄位、沒有重試）
- **點名畫面漏掉請假學生的狀態**——只有「出席／缺席」兩顆，請假學生無狀態
  可選，底部卻說「全部標記完成」
- **點名改了沒存就關閉，零提醒靜默丟棄**；同一個對話框有兩個 ×（右上角、
  班級名旁邊）都關閉整個對話框，看不出差別
- **空值送出全站靜默**（科目、家長表單已確認）；**家長表單填完唯一標星號的
  「姓名」後按鈕仍是灰的**（Email/手機要擇一），畫面沒說還缺什麼

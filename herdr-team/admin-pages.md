...
## 進行中狀態（2026-09-04 —— 這節會過期，接手第一件事：重寫它）

**B3「報名＝開帳」四片全合。** 之後是成績區三刀（#267 不及格形狀訊號、#271 及格門檻
改成總分的 60%、#273 dialog 寬度改 breakpoints）與 #298（未點名卡改由伺服器算）。

**家長端 v1 開了頭。** 探索發現 roadmap 的「空殼」太樂觀 —— **家長身分在整個 API 層
只打得到 `/api/me` 與 `/api/announcements`**，兩支都不回孩子的資料。11 頁前端全是
`empty-state`，但路由與選單完整（21 筆 `PARENT_*`）。骨架在，內臟沒有。

| PR   | 內容                                                | 狀態         |
| ---- | --------------------------------------------------- | ------------ |
| #291 | 家長通知中心（零新 API，收件匣抽進 `shared/`）      | 已合         |
| #295 | **家長端資料範圍模型**（授權設計，`status: draft`） | 待使用者窗口 |
| #304 | API query 參數覆蓋率 gate                           | 待合         |

### 手上這條線：家長端 v1 的第 02/03 片等授權定案

**#295 沒批准之前不要寫第 02 片**（`GET /api/me/children`）—— 那支是授權模型的第一個
實例，後面每一支會照著它抄。三件待使用者裁：`activeRole` 進 API context、
回應欄位的遮蔽清單、`is_primary` 要不要影響可見性。

**已知限制（照現行為出貨，寫在程式碼註解裡）**：`audienceFor` 用角色陣列的優先序
決定身分，所以**同時是老師又是家長的人切到家長身分時看到的仍是老師的公告**。
錯的方向是**靜默漏看**。

### 掛著的

- **報表的 CSV 匯出** —— spec 要明細層欄位，`/api/reports/revenue` 只回聚合。
  不要用聚合資料湊一個欄位對不上 spec 的 CSV
- **金流家族結構上是斷的** —— 七頁只有 `enrollments` 有跨頁導覽。B3 只解了
  「報名→開帳」那一段
- **`payableAmount` 有兩個 feature 目錄的呼叫端** —— 第三個出現就該往 `shared/` 提

### 這一席的節奏

**後端先行，前端接顯示端小追加**；開工前先確認欠著的做了沒。多數 PR 由計畫席與
review-steward 代合，**有順序依賴要自己講**、疊 PR 要轉 Draft。
**零 idle 制**：交付後從 `herdr-team/backlog.md` 認領下一項（backlog 已改刪除制，
只放活項）。

...
- **smoke 排程還沒有被確認真的跑過**（2026-09-05 隨 #336 進 main）。
  GitHub 的排程 workflow 只在預設分支上生效，而**「它會跑」到目前為止仍是一個假設**。

  ```bash
  gh run list --workflow smoke.yml --limit 5    # 有沒有 schedule 觸發的紀錄
  gh label list --search smoke                  # smoke-failure label（已確認存在）
  ```

  **不確認的話，這支工具本身就是它要治的那個病**：一個從來沒跑過的探測，
  跟一個永遠回綠的探測一樣，都會讓人以為有東西在看著。

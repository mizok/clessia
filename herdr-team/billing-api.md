...
## 現在的狀態（2026-09-05 交接 —— 這節會過期，接手第一件事：重寫它）

> **不要相信這一節的「在飛」，用 `gh pr list --state open` 查。**
> 這一節過期過三輪，而寫下的狀態比記得的狀態更容易錯，因為它不會自己更新。

**這是一次 session 輪替的交接**（前一個 session 的 context 用盡）。下面是接手要知道的全部。

### 你的第一件工作：#295 家長端資料範圍的 API 側（主刀歸這一席）

設計文件：[[architecture/parent-data-scope]]（**已過使用者窗口，可以直接動工**）。
**機制節已於預審修正成「乙案」** —— 動工前確認你讀到的是修正後的版本
（summary 裡有「家長端 route 拿不到原始 supabase」那句就是對的版本）。

四片，建議依序：

1. **middleware 注入 `studentScope`** —— `authMiddleware:62` 已經在查 `parents`，
   把 `parent_student_relations` 一起 join。**形狀照抄 `staff_campuses` → `campusScope`**
   （同一支 middleware 的 `:114-122`），不要另起一套。
   - ⚠️ **不是家長 → `null`（不受限）；是家長但沒綁小孩 → `[]`（什麼都看不到）。**
     設計文件說這是「最容易寫錯的地方」，而它剛好是**寫錯了不會報錯**的那種
     （`[]` 當成 `null` 處理 = 沒綁小孩的家長看到全部）。**用測試釘死這兩個分支。**
2. **`childDb` 入口** —— 家長端 route 拿不到 `c.get('supabase')`，只拿得到已綁 scope 的
   查詢入口。**這是乙案的核心**：讓錯的寫法寫不出來，而不是讓它比較麻煩。
   配一個 gate 收尾（禁止家長端檔案出現 `c.get('supabase')`），比照 A15 掃 `ba_*` 的形狀。
3. **`activeRole` 進 context** —— 目前 `audienceFor` 用角色陣列的優先序，於是
   「老師兼家長」切到家長身分時看到的仍是老師的公告（#291 的已知限制）。
4. **`GET /api/me/children`** —— 02 片的 API 半。

**遮蔽欄位的清單**（成績的班級排名、帳單的內部備註…）是窗口三決之一，
**照清單做，不要自己猜哪些該遮**。

### 排程中

- **500 案回查：09-07 之後。** 計畫席持 CF 憑證查 log，這一席分析。
  查詢條件：`POST /api/courses`、回應 body 含 `"code":"SERVER_ERROR"`、
  **不要用 `--status error`**（那是 `return` 出來的 500 不是例外，濾 invocation outcome
  抓不到），用 `--method POST`。命中的話同一筆會有 `[auth] 身分查詢失敗：<表>=<訊息>`，
  直接指出是三支身分查詢的哪一支。**在拿到 log 之前不要結案** ——
  沒被記錄的空白寫在 lessons 上會偽裝成結論。
- `/api/class-logs`、`/api/session-packs` 的查證（低優，#304 gate 標的守備外實例）。

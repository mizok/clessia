...
> 「有人懷疑時跑一次」的工具，而不是常設關卡 —— 但工具沒有觸發條件就等於沒有人記得：
>
> 1. **有測試「莫名其妙」變紅，而沒有人改過那個檔。** #670 就是這個形狀：09-07 綠、09-12 紅、
>    中間沒有任何 commit 碰過它，而第一個中彈的是一支純文件的 PR。
> 2. **動到任何「用日期窗口重建列表」的程式碼之前。** `fillMissingDays` 全 repo 只有一個
>    呼叫端，所以**下一個寫這種寫法的人不會知道它是陷阱** —— 它跟「把 API 回來的清單過濾
>    一下」長得很像，差別在於**窗外的資料不是排到後面，是整筆消失**，於是替身餵進去的
>    fixture 在畫面上完全不存在，而斷言只會說「找不到」。
>
> ⚠️ **web 紅了就看不到 api** —— 這支腳本是 `nx test web … && cd apps/api && vitest run`，
> web 失敗時 `&&` 會把 api 那半整個短路掉。**那是對的行為，但它會讓人以為兩邊都驗過。**
> 要確認兩邊都過，**web 修綠之後再跑一次，或分開跑**。

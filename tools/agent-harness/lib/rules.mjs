 ...
 *   forbid      — regex that must NOT appear in the newly written **code**; omit to block on path alone
 *                 （註解先抹白 —— 見 comments.mjs。「解釋為何不用 90vw」的註解不是違規）
...
export function matchWriteRules(writes, rules) {
  ...
  for (const { filePath, text } of writes) {
    for (const rule of rules) {
      ...
      if (rule.forbid && !new RegExp(rule.forbid, 'm').test(blankComments(text, filePath)))
        continue;

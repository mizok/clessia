...
## 進行中狀態（2026-09-03 —— 這節會過期，接手第一件事：重寫它）

P2 四個功能區收官後是一長串顯示端修正（#100 / #103 / #112 / #122 / #135 / #142 / #162），
接著三件比較大的：**#169 點名收斂成一支實作**（兩份各有對方缺的防線，合成解）、
**#180 設定四合一**（分校/學校/科目/一般收成 `/admin/settings` 四個 tab）、
**#186 B3 第一片**（報名→計費→開帳收成一次）。

design-web 的「刀 3」已收官（`p-tag` 61 → 8）。剩下 8 個是刻意不動的兩批：
匯入預覽是**行動的預測**、teaching-log 摘要 chip 描述的是**集合的問題數** ——
兩者都不是「單一事物的狀態」。**不要順手把它們也換掉。**

### 手上這條線：B3「報名＝開帳」，設計已批准

設計在 artifact（五個決定 + 明確不做），**決定 4 與 5 是綁定批准的**：
分開事務（報名成立、帳單另計）之所以成立，靠的是「殘留看得見」，
而那要一支 `hasInvoice` 查詢撐著 —— 沒有那支就該改成綁事務。三片：

1. **單筆接開帳** —— #186，完成，**必須排在 #184 之後合**
2. **批次結果的開帳入口 + 期中插班試算** —— 待做，等 #184 進 main
3. **待開帳清單** —— 待做，靠 #184 的 `hasInvoice` 篩選

試算的 `note`（「期間 30 天，實際 15 天，比例試算，可調整」）**必須顯示在金額旁** ——
只給數字沒人敢改，那句話是行政決定動不動數字的依據。
...
- **金流家族結構上是斷的** —— 七個頁面裡只有 `enrollments` 有跨頁導覽，其餘六頁連一個
  `routerLink` 都沒有。B3 只解了「報名→開帳」那一段，其餘還在計畫席的池子裡

### 這一輪的節奏

**後端先行，前端接顯示端小追加**；開工前先確認欠著的那些做了沒。
跨席的視覺／token 改動會回頭波及已交付的頁（#95 → #100），design-web 的刀合併後
回去看一眼自己的頁。**另外：合併授權 v2 之後多數 PR 由計畫席代合，
有順序依賴要自己講**（見工作習慣）。

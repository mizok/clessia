...
- **#243** `index.spec` 不再打真資料庫 + `test-setup.ts` 把稽核的靜默失敗變成紅燈

- **#215** executionCtx 收斂（86 處三種寫法，含 3 處「可選鏈假防護」）。
  **排在 #209 之後**，等它合了再 rebase 重解 —— 解衝突時用
  `git checkout origin/main -- <衝突檔>` 取完整版當底再重套，因為
  **均質替換漏掉一處在 diff 上看不出來**，而漏掉的症狀只在沒有 ExecutionContext
  的環境發作（正式站永遠不顯現、測試也不會紅）。解完用
  `grep -rn "executionCtx" apps/api/src | grep -v spec | grep -v lib/wait-until.ts`
  必須是空的當機械證明，不靠注意力。

### 觀察中（不瞎修）

新增課程偶發「第一次 500、第二次 201」。唯一吻合的路徑是 `middleware/auth.ts`
三支身分查詢任一失敗時 `return` 的 500。冷測未能重現。#163 已部署，
下一次發生會在 tail 留下 `[auth] 身分查詢失敗：<表>=<訊息>`。

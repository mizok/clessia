...
## 現在的狀態（2026-09-02 —— 這節會過期，接手第一件事：重寫它）
...
#123（停課與代課資訊）已合併。**#129 中了幽靈合併** —— base 沒轉就被合進 #123 的死分支，
內容從來沒進 main，用 `git cherry-pick` 原封搬成 **#131**（疊 PR 鐵律第 1 條的第二個案例）。

**在飛的（都等使用者親手合）**：

- **#128** 資料庫連線接上 Hyperdrive。**卡在使用者跑 `wrangler hyperdrive create`** ——
  連線字串是機密，agent 不經手。wrangler.toml 的 binding 刻意留註解（理由見上）
- **#131** 課堂列表 `examCount` + 聯絡簿 `/missing/summary` 週彙總（#129 的撿回）
- **#134** 老師任課範圍查空（`schedules` 沒有 `org_id`）+ `schedules.teacher_id` 的
  DEPRECATED 註解除鏽
- **#137** 拔掉 `attendance_records.status` 的預設值 `'absent'`
- **#143** seed 補上出勤測試學生的家長帳號與親子關聯
- **#145** 掃碼不覆蓋人工點名 + `recorded_by_role` 記實際角色

**不進版控的交付**：密集日 demo 資料補丁（`dense-day-seed.sql` + README，在 scratchpad），
規格來自 design-web，由人拿去對 demo 執行。

...
## design-web
1. 時間軸濃度圖設計稿 [認領 bundle-analysis-4f 11:37](N=3 的真解:每半小時一根、濃度=同時堂數 —— 換畫法,day-timeline.util 重做的設計不動工)
2. bundle lazy chunk 內部拆分(你封存的舊分析的後續,撿回來評估哪些值得)
3. (窗口批准後)作業台實作

## design-web-2
1. 拇指區 C 期(#191 完成)→ D 期 gate(頁面層級 p-button 不得直寫 __header-actions) [認領 design-web-2-df 11:39]
